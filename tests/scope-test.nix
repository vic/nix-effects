...
let
  ...
  twoUsersTest = {
    expr =
      let
        comp =
          bind (scope.const "user" "alice" greet) (a:
          bind (scope.const "user" "bob"   greet) (b:
            pure { alice = a; bob = b; }));
      ...
      let
        comp = bind getUser (user:
          bind (send "log" user) (_:
            pure user));
        scoped = scope.const "user" "alice" comp;
        result = handle {
      ...
      let
        comp = bind (send "env" null) (env:
          bind (send "user" null) (user:
            pure "${env}/${user}"));
        inner = scope.const "user" "bob" comp;
        outer = scope.const "env" "prod" inner;
      ...
      let
        comp =
          bind (send "getHandler" null) (userName:
            scope.const "user" userName getUser);
        result = handle {
  ...
  threeUsersFanOut = {
    expr =
      let
        ...
        perUser = builtins.map (u: scope.const "user" u greet) users;
    ...
    expr =
      let
        comp = bind getUser (u1:
          bind (scope.const "user" "inner" getUser) (u2:
            pure { outer = u1; inner = u2; }));
        scoped = scope.const "user" "outer" comp;
